...
export async function deploy(actionContext: IActionContext, arg1?: vscode.Uri | SiteTreeItem, arg2?: (vscode.Uri | SiteTreeItem)[], isNewApp: boolean = false): Promise<void> {
    ...
    if (siteConfig && javaUtils.isJavaRuntime(siteConfig)) {
        ...
        module = javaUtils.getMavenModule(context.effectiveDeployFsPath);
    ...
    if (javaUtils.isJavaRuntime(siteConfig) && module) { // modify deployPath to the executable jar or war files generated by preDeployTask when deploying maven module
        const moduleFolder = path.dirname(module.pom);
