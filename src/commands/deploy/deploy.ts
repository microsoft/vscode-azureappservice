...
export async function deploy(actionContext: IActionContext, arg1?: vscode.Uri | SiteTreeItem, arg2?: (vscode.Uri | SiteTreeItem)[], isNewApp: boolean = false): Promise<void> {
    ...
    let fileExtensions: string | string[] | undefined = await javaUtils.getJavaFileExtensions(siteConfig);
    ...
    if (siteConfig && javaUtils.isJavaRuntime(siteConfig)) {
        ...
        const ext = javaMavenModule?.packaging ?? context.effectiveDeployFsPath.split('.').pop() ?? '';
        ...
        javaMavenModule && await setPreDeployTaskForMavenModule(context, javaMavenModule);
    ...
    let deployPath: string = isZipDeploy ? context.effectiveDeployFsPath : context.originalDeployFsPath;
    if (javaMavenModule) { // modify deployPath to the executable jar or war files generated by preDeployTask when deploying maven module
        const moduleFolder = path.dirname(javaMavenModule.pom);
