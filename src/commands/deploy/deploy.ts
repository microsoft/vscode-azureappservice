...
export async function deploy(context: IActionContext & Partial<IDeployWizardContext>, confirmDeployment: boolean, target?: Uri | SiteTreeItem | undefined): Promise<void> {
    ...
    context.deployFsPath = await getDeployFsPath(context, target, javaFileExtension);
    context.workspace = workspaceUtil.getContainingWorkspace(context.deployFsPath);

    if (!context.workspace) {
        throw new Error('Failed to deploy because the path is not part of an open workspace. Open in a workspace and try again.');
    }

    const projectRootFsPath: string = context.workspace.uri.fsPath;
    ...
    if (!node) {
        ...
        try {
            // the workspace is required to set by here
    ...
    if (getWorkspaceSetting<boolean>(constants.configurationSettings.showBuildDuringDeployPrompt, projectRootFsPath)) {
        //check if node is being zipdeployed and that there is no .deployment file in the root folder
        if (siteConfig.linuxFxVersion && siteConfig.scmType === 'None' && !(await pathExists(path.join(projectRootFsPath, constants.deploymentFileName)))) {
            ...
            if (linuxFxVersion.startsWith(appservice.LinuxRuntimes.node)) {
                ...
                await node.promptScmDoBuildDeploy(projectRootFsPath, appservice.LinuxRuntimes.node, context);
            } else if (linuxFxVersion.startsWith(appservice.LinuxRuntimes.python)) {
                await node.promptScmDoBuildDeploy(projectRootFsPath, appservice.LinuxRuntimes.python, context);
    ...
    if (confirmDeployment && siteConfig.scmType !== constants.ScmType.LocalGit && siteConfig !== constants.ScmType.GitHub) {
        ...
        if (result === resetDefault) {
            const settingsPath = path.join(projectRootFsPath, '.vscode', 'settings.json');
            const doc = await workspace.openTextDocument(Uri.file(settingsPath));
            window.showTextDocument(doc);
            await updateWorkspaceSetting(constants.configurationSettings.defaultWebAppToDeploy, '', projectRootFsPath);
    ...
    if (!context.deployedWithConfigs) {
        ...
        node.promptToSaveDeployDefaults(projectRootFsPath, context.deployFsPath, context);
    }
