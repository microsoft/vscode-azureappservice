...
export async function deploy(actionContext: IActionContext, arg1?: vscode.Uri | SiteTreeItem, arg2?: (vscode.Uri | SiteTreeItem)[], isNewApp: boolean = false): Promise<void> {
    ...
    if (javaUtils.isMavenModule(context.effectiveDeployFsPath)) { // modify deployPath to the executable jar or war files generated by preDeployTask when deploying maven module
        const executableArtifacts = await javaUtils.getExecutableArtifactsByExtensions(path.join(context.effectiveDeployFsPath, 'target'), fileExtensions);
