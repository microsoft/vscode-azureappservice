...
import { ColumnName, detectorResponseJSON, detectorTable, validateTimestamp } from "../src/commands/postDeploy/checkLinuxWebAppDownDetector";
...
suite('Detector Dataset Parser', () => {
    ...
    test('Verify validateTimestamp', async () => {
        ...
        const staleTimestamp: Date = new Date("7/26/2019 8:40:25 PM UTC ");
        const table: detectorTable | undefined = findTableByRowValue(detectorJSONWithCriticalErrors.properties.dataset, vsCodeIntegration);
        assert.ok(table);
        const parsedTimestamp: Date = new Date(getValuesByColumnName(nonNullValue(table), ColumnName.value)[0]);
        assert.equal(validateTimestamp(parsedTimestamp, timestamp), true);
        assert.equal(validateTimestamp(parsedTimestamp, staleTimestamp), false);
    });
    ...
    test('Get failure counts', async () => {
        const table: detectorTable | undefined = findTableByColumnName(detectorJSONWithCriticalErrors.properties.dataset, ColumnName.failureCount);
        assert.ok(table);
        const failureCount: string = getValuesByColumnName(nonNullValue(table), ColumnName.failureCount)[0];
    ...
    test('Get error messages', async () => {
        ...
        assert.ok(table);
        const errorMessages: string[] = getValuesByColumnName(nonNullValue(table), ColumnName.value);
        const expectedErrorMessage: string = 'Your app failed to start almost immediately.';
        assert.equal(errorMessages[0], expectedErrorMessage);
    });
});
